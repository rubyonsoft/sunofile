...
> 위 이미지는 다운로드 제한 정책을 이해하기 위한 참고 자료입니다. 이미지에 표시된 시행일과 요금제별 횟수는 Suno 사정에 따라 변경될 수 있으므로, 실제 적용 내용은 [Suno 공식 도움말](https://help.suno.com/)과 본인 계정의 구독 화면에서 다시 확인하세요. 현재 Suno 공식 도움말에서는 WAV 다운로드가 Pro 및 Premier 구독자에게 제공된다고 안내합니다.

이 프로그램은 다운로드가 중단되거나 일부 곡이 실패하더라도 기록을 저장합니다. 나중에 다시 실행하면 정상적으로 받은 파일은 건너뛰고 빠진 파일만 이어서 시도합니다.
