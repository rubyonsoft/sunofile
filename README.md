...
- 선택한 음악 파일 개수로 완료된 Workspace를 빠르게 건너뛰는 선택 옵션
...
3. `Suno-Backup-Studio-Setup-3.1.1-x64.exe`를 누릅니다.
...
1. 받은 `Suno-Backup-Studio-Setup-3.1.1-x64.exe`를 두 번 클릭합니다.
...
- **파일 개수로 완료된 Workspace 빠르게 건너뛰기**: 선택한 MP3/WAV 파일 수가 Suno에 표시된 곡 수 이상인 Workspace는 곡 목록을 열지 않고 생략
...
설정 화면에서 저장 버튼을 누르지 않고 대시보드로 이동해도, **곡 목록 확인**이나 **전체 백업 시작**을 누르는 순간 화면에 보이는 저장 폴더와 옵션이 자동으로 저장되어 해당 작업에 적용됩니다. 작업 기록의 `백업 저장 폴더`와 `빠른 Workspace 건너뛰기` 줄에서 실제 적용값을 확인할 수 있습니다.

### 파일 개수로 빠르게 건너뛰기 옵션

곡이 많은 계정에서 다시 백업할 때 시간을 줄이기 위한 선택 기능이며 기본값은 **꺼짐**입니다. 설정 화면에서 옵션을 켜고 저장한 뒤 **전체 백업 시작**을 누르면 다음처럼 판단합니다.

- `MP3만`: Workspace 폴더의 MP3 수가 Suno 표시 곡 수 이상이면 건너뜀
- `WAV만`: Workspace 폴더의 WAV 수가 Suno 표시 곡 수 이상이면 건너뜀
- `MP3 + WAV`: MP3와 WAV 수가 각각 Suno 표시 곡 수 이상일 때만 건너뜀

이 옵션은 빠른 대신 파일 이름이나 곡 ID, 가사, 프롬프트, 메타데이터의 존재 여부를 검사하지 않습니다. Suno에서 삭제한 옛 곡의 파일이 PC 폴더에 남아 있으면 파일 수는 충분해 보여도 새 곡이 빠질 수 있습니다. 정확하게 새 곡과 부가 파일을 확인하려면 옵션을 끄세요. **곡 목록 확인** 기능에서는 이 옵션을 사용하지 않고 항상 Suno 목록을 확인합니다.
...
수동 실행에서 빠른 건너뛰기를 사용하려면 `config.json`을 메모장으로 열고 아래 값을 `true`로 바꿉니다. 정확한 확인이 필요할 때는 다시 `false`로 바꾸세요.
